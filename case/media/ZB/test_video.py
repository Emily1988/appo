...
class Video(unittest.TestCase,BaseCase):
    ...
    def confirm_replayer(self):
        if self.driver.page_source.find('上次观看至') != -1:
    ...
    '''
    def goto_video_player(self):
        self.confirm_replayer()
        try:
            print('goto_video_player')
            ...
            while True:
                if self.driver.page_source.find('{}'.format(self.filename)) == -1:
                    time.sleep(4)
    ...
    def seclet_player_open_file(self):
        if self.driver.page_source.find('仅此一次') != -1:
            self.driver.press_keycode(20) #出现播放器列表
            self.driver.find_element_by_xpath("//*[@text='视频播放器']").click()
            if self.driver.page_source.find('仅此一次') != -1:
                self.driver.find_element_by_xpath("//*[@text='始终']").click()

        if self.driver.page_source.find('使用视频播放器打开') != -1:
            self.driver.find_element_by_xpath("//*[@text='始终']").click()
    ...
    def handle_walk_smple(self,num):
        ...
        try:
            for i in range(0, num):
                print(templist[i])
                # self.driver.find_element_by_xpath("//*[@text='{}']".format(templist[i])).click()
    ...
    def toast_exist(self):
        try:
            if self.driver.page_source.find('不能播放此视频') != -1:
                self.driver.find_element_by_xpath("//*[text='确定']").click()
                return True
            else:
                return False
    ...
    #进入音频播放音乐
    def test_scanVideo(self,n):
        self.move_down_until_element_appear('视频格式测试')
        self.driver.find_element_by_xpath("//*[@text='视频格式测试']").click()
        # 做下面的操作让其必须在第一个位置
        self.driver.long_press_keycode(21)
        self.driver.long_press_keycode(21)
        self.driver.long_press_keycode(21)
...
if __name__ == '__main__':
    ...
    p.test_scanVideo(len(audiolist))
